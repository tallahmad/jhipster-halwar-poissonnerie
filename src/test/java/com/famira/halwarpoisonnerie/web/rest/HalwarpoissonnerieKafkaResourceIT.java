package com.famira.halwarpoisonnerie.web.rest;
...
import com.famira.halwarpoisonnerie.IntegrationTest;
import com.famira.halwarpoisonnerie.config.EmbeddedKafka;
import com.famira.halwarpoisonnerie.config.KafkaSseConsumer;
import com.famira.halwarpoisonnerie.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class HalwarpoissonnerieKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/halwarpoissonnerie-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/halwarpoissonnerie-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/halwarpoissonnerie-kafka/unregister"));
                return;
            }
        }
